            ...
            <div class="container">
                ...
                <div class="collapse navbar-collapse" id="navbarNav">
                    <ul class="navbar-nav ms-auto">
                        ...
                        <li class="nav-item">
                            <a class="nav-link active" href="vol-opp.php">Opportunities</a>
                        </li>
        ...
        <section class="p-5">
            <div class="border-bottom p-2 bg-light mb-3">
                <div class="row list" style="text-align:center">
                    <form class="filterform" method="post" action="vol-opp.php">
            ...
            <div class="row list">
                <div class="col-md-12">
                    ...
                    <?php
                        ...
                        if(isset($_POST['filter'])){
                            ...
                            $sql2 .= " WHERE (name LIKE '%{$filter}%' OR location LIKE '%{$filter}%' OR skill LIKE '%{$filter}%' OR description LIKE '%{$filter}%'
                                    OR startdate LIKE '%{$filter}%' OR enddate LIKE '%{$filter}%' OR vol_num LIKE '%{$filter}%' or vol_found LIKE '%{$filter}%' or org_name LIKE '%{$filter}%') AND status = 'open'";
                            $_SESSION['filter'] = $filter;
                        }else{
                            $sql2 .= " WHERE status = 'open'";
                        }
                    ...
                    <!-- Opp list -->
                            ...
                            <?php
                            $sql3 = "SELECT * FROM application WHERE (vol_email='$email' AND oppid = '$oppid')";
                            $result3 = $con->query($sql3);
                            if ($result3->num_rows > 0) {
                                $row3 = $result3->fetch_assoc();
                                $status = $row3['status'];
                                //volunteer has already applied  
                            } else if ($vol_found >= $vol_num){ ?>
                                <div class="d-flex mb-3 pb-3 border-bottom">
                                    <div class="ms-3 ms-md-4">
                                        <input  type="submit" name="apply" class="custom-btn btn" style="float:right" disabled value="Limit Reached"></input>
                                        <h3><?php echo $name; ?></h3>
                                ...
                                <div class="d-flex mb-3 pb-3 border-bottom">
                                    <div class="ms-3 ms-md-4">
                                        <a href="vol-applyopp.php?apply=<?php echo $oppid ?>" class="custom-btn btn" style="float:right">Apply</a>
                                        <h3><?php echo $name; ?></h3>
                                        <p class="mb-2 text-dark text-opacity-75">Location: <?php echo $location; ?></p>
                                        <p class="mb-2 text-dark text-opacity-75">Organization: <?php echo $org_name; ?></p>
                                        <p class="mb-2 text-dark text-opacity-75">Skills needed: <?php echo $skill; ?></p>
